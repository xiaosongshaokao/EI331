...
class char_cnn_net:
    def __init__(self):
        self.dataset = numbers + alphabets
    ...
    def cnn_construct(self):
        ...
        cw2 = tf.Variable(tf.random_normal(shape=[3, 3, 32, 64]
    ...
    def test(self, test_dir, model_path):
        # text_list = []
        x_images, actual_y = self.init_testData(test_dir)
        out_put = self.cnn_construct()
        predicts = tf.nn.softmax(out_put)
        predicts = tf.argmax(predicts, axis=1)
        actual_y = tf.argmax(actual_y, axis=1)
        accuracy = tf.reduce_mean(tf.cast(tf.equal(predicts, actual_y), dtype=tf.float32))
        saver = tf.train.Saver()
        with tf.Session() as sess:
            ...
            acc, preds = sess.run([accuracy, predicts], feed_dict={self.x_place: x_images, self.keep_place: 1.0})
    ...
    def init_data(self, dir):
        ...
        for file in files:
            ...
            # 仅需要数字与字母部分数据
            if dir_name not in self.dataset:
                continue
    ...
    def init_testData(self, dir):
        ...
        for file in files:
            ...
            index_y = self.dataset.index(dir_name)
            vector_y[index_y] = 1
            actual_Y.append(vector_y)
        test_X = np.array(test_X)
        actual_Y = np.array(actual_Y)
        return test_X, actual_Y
...
if __name__ == '__main__':
    ...
    train_model_path = os.path.join(cur_dir, r'carIdentityData\model\numAndLetter_recognize\model.ckpt')
    model_path = os.path.join(cur_dir, r'carIdentityData\model\numAndLetter_recognize')
    ...
    if train_flag == 1:
        ...
        # 测试部分
        acc, preds = net.test(test_dir, model_path)
        print(acc)
