...
class plate_cnn_net:
    ...
    def train(self, data_dir, model_save_path):
        print('ready load train data set')
        X, y = self.init_data(data_dir)
        print('success load ' + str(len(y)) + ' data')
    ...
    def test(self, test_dir, model_path):
        x_images, actual_Y = net.init_testData(test_dir)
        out_put = self.cnn_construct()
        predicts = tf.nn.softmax(out_put)
        ...
        with tf.Session() as sess:
            ...
            acc, preds, probs = sess.run([accuracy, predicts, probabilitys], feed_dict={self.x_place: x_images, self.keep_place: 1.0})
    ...
    def init_data(self, dir):
        ...
        map_label = {"has": 1, "no": 0}
        labels = [map_label[os.path.split(os.path.dirname(file))[-1]] for file in files]
        ...
        for i, file in enumerate(files):
            ...
            X.append(resize_img)
            y.append([[0, 1] if labels[i] else [1, 0]])
...
if __name__ == '__main__':
    cur_dir = sys.path[0]
    data_dir = os.path.join(cur_dir, r'carIdentityData\cnn_plate_train')
    test_dir = os.path.join(cur_dir, r'carIdentityData\cnn_plate_test')
    ...
    if train_flag == 1:
        ...
        # 测试部分
        # test_X, actual_Y = net.init_testData(test_dir)
        acc, preds, probs = net.test(test_dir, model_path)
        print(acc)
